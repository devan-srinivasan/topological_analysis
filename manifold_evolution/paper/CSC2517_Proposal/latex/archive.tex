...
vectors than the rank of the vector space is called superposition. Under this principle in the context of LLM's, as each token is processed in parallel, 
...
Now given this hypothesis, we can create a new interpretation on the efficacy of prompting strategies like LeastToMost \cite{zhou2023leasttomostpromptingenablescomplex} and many shot in-context learning \cite{agarwal2024manyshotincontextlearning}. Since LLMs
are, by design, constrained to work with the content present in each token's stream, when different tokens are present in the prompt, it
...
with, yielding a different (hopefully better) answer. Colloquially this phenomena has been described differently as "jumping manifolds" where
...
\begin{enumerate}
  ...
  \item Formalize theoretical ideas behind "jumping manifolds" under the linear representation hypothesis
...
One avenue of applying tools from topology to machine learning problems has demonstrated the relationship bewtween the complexity of the problem and/or model, and it's topological features.
...
\cite{fitz2024hiddenholestopologicalaspects} uses various tools from TDA on point cloud residual manifolds in transformer language models, studying topological change
...
Motivated by prior work we seek to investigate the theoretical mathematics behind residual manifolds, and "jumping" as it relates to topology and the linear representation hypothesis. 
...
$Z = A F$ where $A \in \mathbb{R}^{n \times m}$ is a sparse matrix.
...
Under LRH we can formalize this notion of "jumping" as follows. Given a pair of point cloud of latent representations $Z_1, Z_2$ based on token sets $T_1, T_2$ respectively, (where perhaps $T_2$ is a "better" prompt than $T_1$)
...
\begin{enumerate}
  \item Using simplicial and/or persistent homology, we can build Vietoris-Rips complexes on $Z$ and measure Betti numbers $\beta_k$ to capture global topological structure. Can we develop a theoretical connection to "jumps"?
...
\subsubsection{Residual Topology}
Prior work has undoubetdly demonstrated semantic correlation in how tokens are embedded (\citet{word2vec} is one), can we relate this to how semantically correlated point clouds may look topologically? 
Assuming we build a simplicial complex from this cloud, are there any theoretical suggestions to the structures homology class, or persistent homology?
...
To this end we chose gradeschool level math and science concept explanations, which can be easily sourced from Wikipedia. This is advantageous
...
we are simply interested in topology not necessarily the actual distribution over the type of text samples, so this should suffice.
